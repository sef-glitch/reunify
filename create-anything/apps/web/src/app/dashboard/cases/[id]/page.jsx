...
function TasksTab({ tasks, onAddTask, onEditTask }) {
  return (
      ...
      <div className="space-y-3">
        {tasks?.map((task) => (
          <div
            ...
            className="flex items-center gap-3 p-4 bg-white border border-gray-200 rounded-xl cursor-pointer hover:border-purple-300 hover:shadow-sm transition-all"
          >
            <div
              className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${task.status === "completed" ? "bg-green-500 border-green-500 text-white" : "border-gray-300"}`}
            >
...
function EditTaskModal({ task, onClose, onSubmit, isLoading, error }) {
  const [formData, setFormData] = useState({
    id: task.id,
    title: task.title || "",
