...
import { DEMO_CASE, DEMO_TASKS, DEMO_DOCUMENTS, DEMO_EVIDENCE } from './demo-data';
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    async function init() {
      try {
        initDatabase();
        const storedUserId = await SecureStore.getItemAsync(USER_SESSION_KEY);
        if (storedUserId) {
  ...
  const signIn = useCallback(async (email: string, password: string): Promise<{ success: boolean; error?: string }> => {
    ...
    await SecureStore.setItemAsync(USER_SESSION_KEY, authenticatedUser.id);
  ...
  const seedDemoData = useCallback(() => {
    ...
    const alreadySeeded = getSetting(user.id, 'demo_data_seeded');
    if (alreadySeeded === 'true') return;

    // Create demo case
    const demoCase = dbCreateCase(user.id, {
      name: DEMO_CASE.name,
      state: DEMO_CASE.state,
      county: DEMO_CASE.county,
      caseType: DEMO_CASE.caseType,
      stage: DEMO_CASE.stage,
      nextHearingDate: DEMO_CASE.nextHearingDate,
      goals: DEMO_CASE.goals,
    });
    ...
    // Mark as seeded
    setSetting(user.id, 'demo_data_seeded', 'true');
